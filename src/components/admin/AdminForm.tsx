...
import { useGetRoleQuery } from "@/redux/api/roleApi/roleApi";
import { Role } from "@/types/role";
import { useCreateAdminMutation, useGetAdminByIdQuery, useUpdateAdminDetailsMutation } from "@/redux/api/adminApi/adminApi";
import toast from "react-hot-toast";
import { Admin } from '@/types/admin';
...
const AdminForm = () => {
     ...
     const onSubmit: SubmitHandler<Admin> = async (data) => {
          ...
          try {
               if (formattedText === "create") {
                    ...
                    const result = await updateAdminDetails({ formData, id }).unwrap();
     ...
     return (
          <Card className="p-4 rounded-sm gap-4 shadow-none">
               {
                    formattedText === "update" ? <h1 className="text-xl font-semibold">Upate Admin Info</h1> : <h1 className="text-xl font-semibold">Create Admin</h1>
               }
                    ...
                    <div className="flex flex-col items-center justify-center border border-gray-200 rounded-sm p-4 relative">
                         <div className="flex flex-col items-center">
                              ...
                              {imagePreview ? (
                                   ...
                                   // If preview is not selected but adminInfo.image exists
                                   <div className="relative">
                                        <img
                                             src={adminInfo.image}
                                             alt="Profile"
                                   ...
                                   // If nothing available → placeholder
